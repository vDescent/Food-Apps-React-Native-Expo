...
const Progress2b = () => {
    ...
    PanResponder.create({
      ...
      onPanResponderRelease: (evt, gestureState) => {
        if (gestureState.dx < -50) {
          router.push('/(progress)/progress2c' as any);
  ...
  useEffect(() => {
    const fetchStep1 = async () => {
      try {
        ...
        const q = query(langkahRef, where('compare', '==', 'hamburger'));
        const querySnapshot = await getDocs(q);
        if (!querySnapshot.empty) {
          const doc = querySnapshot.docs[0]; 
          setStepText(doc.data().step2 || 'data tidak ada di database.');
        } else {
  ...
  return (
    <ImageBackground
      source={require('@/assets/images/img_progress2.jpg')}
    ...
    >
      ...
      <View style={styles.instruksi} {...panResponder.panHandlers}>
        <Text style={styles.text_progress}>Langkah 2</Text>
