    ...
    PanResponder.create({
      ...
      onPanResponderRelease: (evt, gestureState) => {
        if (gestureState.dx < -50) {
          router.push('/(progress)/progress2g' as any);
    ...
    const fetchStep1 = async () => {
      try {
        ...
        if (!querySnapshot.empty) {
          ...
          setStepText(doc.data().step6 || 'data tidak ada di database.');
        } else {
  ...
  return (
    ...
    >
      ...
      <View style={styles.instruksi} {...panResponder.panHandlers}>
        <Text style={styles.text_progress}>Langkah 6</Text>
